...
async def test_login(ac: AsyncClient):
    req1 = await ac.post(f'/auth/login', params={'email': "3@example.com"})
    assert req1.status_code == 200
    req2 = await ac.get(f'/auth/email-login', params={'email': "3@example.com", 'code': req1.json()})
    assert req2.status_code == 200
    req2 = await ac.get(f'/auth/email-login', params={'email': "3@example.com", 'code': 234242424234})
    assert req2.status_code == 400
...
async def test_refresh_token(ac: AsyncClient):
    ...
    req2 = await ac.get(f'/auth/email-login', params={'email': "3@example.com", 'code': req1.json()})
    assert req2.status_code == 200
    req1 = await ac.get(f'/auth/refresh-token', params={'token': req2.json()['refresh_token']})
    assert req1.status_code == 200
    req1 = await ac.get(f'/auth/refresh-token', params={'token': 'efefefefef'})
