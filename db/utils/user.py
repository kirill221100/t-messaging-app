...
def get_user_by_username_sync(username: str, session: Session):
    return (session.execute(select(User).filter_by(username=username))).scalar_one_or_none()
...
def get_user_by_email_sync(email: EmailStr, session: Session):
    return (session.execute(select(User).filter_by(email=email))).scalar_one_or_none()
...
def get_user_by_id_sync(user_id: int, session: Session):
    return (session.execute(select(User).filter_by(id=user_id))).scalar_one_or_none()
...
async def update_online_and_get_session_sync(token=Depends(get_current_user), session: Session = Depends(get_sync_session)):
    user = get_user_by_id_sync(token['user_id'], session)
    user.last_time_online = datetime.datetime.utcnow()
    session.commit()
    return session
