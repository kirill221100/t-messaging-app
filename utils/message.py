...
async def get_messages_by_chat_id_func(chat_id: int, user_id: int, session: AsyncSession,
    ...
    if await check_if_user_in_chat(chat_id, user_id, session):
        ...
        if my_read_date := await get_read_date(chat_id, user_id, session):
            my_read_date = my_read_date.date
        if others_read_date := await get_read_date_from_others(chat_id, user_id, session):
            others_read_date = others_read_date.date
